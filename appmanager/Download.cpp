...
int Download::download()
{
	...
	if (response.isOk())
	{
		std::cout << "[DL] " << response.getStatus() << ": downloaded the thing to " << saveDir << std::endl;

		if (saveFile.empty())
			saveFile = remoteFilename;

		std::ifstream fileContent(GBL::DIR::cache + remoteDirectory + remoteFilename, std::ios::binary);
		if (fileContent.is_open())
		{
			fileBuffer = (std::string((std::istreambuf_iterator<char>(fileContent)), std::istreambuf_iterator<char>()));

			std::cout << "[DL] downloaded file, wrote to buffer: " << getAppropriateFileSize(fileBuffer.size(), 3) << std::endl;
		...
		{
			std::cerr << "[DL] failed to write file to destination" << std::endl;
			return Status::Fail;
		}
	}
	else
