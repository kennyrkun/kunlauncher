...
MultilineInputBox::MultilineInputBox(float width, unsigned int lines) : m_box(SFUI::Box::Type::Input), m_cursor_pos(0), m_max_characters(-1), m_blink_period(1.0f)
{
	m_box.setSize(width, ((SFUI::Theme::getLineSpacing()) * lines) + (SFUI::Theme::PADDING * 2));
//	m_box.setSize(width, SFUI::Theme::getBoxHeight() * lines);
...
{
	m_box.setSize(m_box.getSize().y, SFUI::Theme::getBoxHeight() * lines);
}

unsigned int MultilineInputBox::getLines() const
{
	return (m_box.getSize().y / SFUI::Theme::getBoxHeight());
}
...
void MultilineInputBox::onKeyPressed(sf::Keyboard::Key key)
{
	...
	/*
	case sf::Keyboard::Up:
		m_text.move(0, SFUI::Theme::getBoxHeight());
		break;
...
void MultilineInputBox::onStateChanged(SFUI::State state)
{
...
void MultilineInputBox::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	glScissor((pos.x + SFUI::Theme::borderSize) - 1, target.getSize().y - (pos.y + getSize().y) + 1, getSize().x - 2, getSize().y - 2);
