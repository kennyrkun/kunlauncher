...
void InitialiseState::Cleanup()
{
	...
	std::cout << "IntialiseState Cleanup" << std::endl;
}
	...
	while (app->window->pollEvent(event))
	{
		...
		else if (event.type == sf::Event::EventType::KeyPressed)
		{
			if (event.key.code == sf::Keyboard::Key::R)
			{
				...
				helperThread = new std::thread(&InitialiseState::loadApps, this);
...
void InitialiseState::loadApps() // TOOD: this.
{
...
void InitialiseState::updateScrollThumb()
{
	// set the scrollbar size
...
bool InitialiseState::mouseIsOver(sf::Shape &object)
{
...
bool InitialiseState::mouseIsOver(sf::Text &object)
{
	if (object.getGlobalBounds().contains(app->window->mapPixelToCoords(sf::Mouse::getPosition(*app->window), *cardScroller)))
		return true;
	else
